...
def get_aupr(y_true, y_pred, threshold=None):
    y_true = np.array(y_true)
    y_pred = np.array(y_pred)
    if threshold is not None:
        y_true = (y_true > threshold).astype(int)  # 根据阈值二值化
...
def get_auc(y_true, y_pred):
    ...
    # 必须保证y_true有两个类别，否则roc_auc_score会报错
    if len(np.unique(y_true)) < 2:
        # 只有一种类别时，auc没有意义，返回nan或0
        return float('nan')
